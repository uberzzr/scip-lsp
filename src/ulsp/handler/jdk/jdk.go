...
func (h *handler) ResolveBreakpoints(ctx context.Context, req *pb.ResolveBreakpointsRequest) (*pb.ResolveBreakpointsResponse, error) {
	...
	res, err := h.jdk.ResolveBreakpoints(ctx, mapper.ResolveBreakpointsRequestToResolveBreakpoints(req, workspaceRoot))
	return mapper.BreakpointLocationsToBreakpointsResponse(res), err
}
...
func (h *handler) ResolveClassToPath(ctx context.Context, req *pb.ResolveClassToPathRequest) (*pb.ResolveClassToPathResponse, error) {
	call := yarpc.CallFromContext(ctx)
	workspaceRoot := call.Header(_gpdWorkspaceRootHeader)
	sourceURI, err := h.jdk.ResolveClassToPath(ctx, mapper.ResolveClassToPathRequestToResolveClassToPath(req, workspaceRoot))
	return mapper.SourceURIToClassToPathResponse(sourceURI), err
}
